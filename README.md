...
backend/     ASP.NET Core 8 Web API (C#) implementing that schema via EF
             ...
             See backend/README.md for architecture and how it was verified.
frontend/    React + TypeScript web client covering every module the
             ...
             See frontend/README.md for architecture and how it was verified.
```
...
dependency on the internet or a shared backend. The built frontend
(`npm run build` → `frontend/dist/`) is meant to be served from the same
origin as the API in production — see `backend/README.md`'s own
**Deployment model** section for the reasoning (and why SQL Server was
chosen for this in the first place, in `database/00_README.md`).

## Status

Every module the schema defines has a working backend endpoint, and every
one of those now has a working frontend screen — nothing left as a
placeholder. Both layers were validated live: the backend against a real
SQL Server 2022 instance (migrations, constraints, and full business
transactions run and hand-verified), and the frontend against the real
running backend in an actual browser (Playwright), with money math
hand-checked before trusting the screen's own numbers. See each
sub-project's README for the detailed verification record. Automated test
suites for both layers are the natural next step — everything so far was
validated by hand against real, running systems rather than left unverified.
